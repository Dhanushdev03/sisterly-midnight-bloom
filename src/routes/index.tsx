...
function Lightbox({ index, close, move }: { index:number; close:()=>void; move:(n:number)=>void }) { const activePhoto = photos[index]; if (!activePhoto) return null; return <div className="fixed inset-0 z-[70] grid place-items-center bg-ink/90 p-4 backdrop-blur-xl" role="dialog" aria-modal="true" aria-label="Photo viewer"><Button variant="glass" size="icon" onClick={close} className="absolute right-4 top-4 z-10" aria-label="Close photo"><X/></Button><Button variant="glass" size="icon" onClick={() => move(-1)} className="absolute left-3 top-1/2 z-10 -translate-y-1/2" aria-label="Previous photo"><ChevronLeft/></Button><figure className="animate-scale-in max-h-[88svh] max-w-5xl"><img src={activePhoto.src} alt={activePhoto.alt} className="max-h-[78svh] max-w-full rounded-md object-contain shadow-2xl"/><figcaption className="mt-4 text-center font-hand text-xl text-foreground/80">{activePhoto.caption}</figcaption></figure><Button variant="glass" size="icon" onClick={() => move(1)} className="absolute right-3 top-1/2 z-10 -translate-y-1/2" aria-label="Next photo"><ChevronRight/></Button></div>; }
...
function MusicControl() {
  const [playing,setPlaying] = useState(false); const [muted,setMuted] = useState(false); const ctx = useRef<AudioContext | null>(null); const volume = useRef<GainNode | null>(null); const timer = useRef<number | null>(null);
  ...
  const play = () => { const AudioCtx = window.AudioContext || window.webkitAudioContext; const audio = new AudioCtx(); const master = audio.createGain(); master.gain.value = muted ? 0 : 1; master.connect(audio.destination); volume.current=master; ctx.current=audio; let step=0; const notes=[261.63,329.63,392,523.25,392,329.63]; const chime=()=>{ const osc=audio.createOscillator(); const gain=audio.createGain(); osc.type="sine"; osc.frequency.value=notes[step%notes.length] ?? 261.63; gain.gain.setValueAtTime(0.0001,audio.currentTime); gain.gain.exponentialRampToValueAtTime(0.035,audio.currentTime+.08); gain.gain.exponentialRampToValueAtTime(0.0001,audio.currentTime+1.4); osc.connect(gain).connect(master); osc.start(); osc.stop(audio.currentTime+1.5); step++;}; chime(); timer.current=window.setInterval(chime,900); setPlaying(true); };
  useEffect(() => () => { if(timer.current) window.clearInterval(timer.current); ctx.current?.close(); }, []);
  useEffect(() => { if(volume.current && ctx.current) volume.current.gain.setTargetAtTime(muted ? 0 : 1, ctx.current.currentTime, .05); }, [muted]);
